<template>
  <div class="container">
    ...
    <!-- 新增按钮、批量上传、下载模版 -->
    <div class="buttonDiv">
      <el-button type="primary" icon="plus" @click="openAddDialog">新增组长</el-button>
    ...
    <!-- 中部表格显示组长信息 -->
    <div class="tableDiv">
      <el-table :data="filteredLeaderData" border style="width: 100%">
...
const filteredLeaderData = computed(() => {
  return tableData.filter(emp => {
    // 只显示组长
    if (emp.employeeRole !== 'leader') return false
...
const handleStatusChange = (row) => {
  ...
  ElMessageBox.confirm(`确定要将组长 ${row.employeeName} 设置为${actionText}状态吗?`, '提示', {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定要删除组长 ${row.employeeName} 吗?`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
